...
class VPSA(nn.Module):
    def __init__(self, channel, out_channel, flag=0):
        ...
        self.theta_x_alpha = nn.Sequential(nn.Conv2d(channel, channel, 1, 1, bias=False), nn.BatchNorm2d(channel),
                                           nn.ReLU(inplace=True))
        self.theta_x_beta = nn.Sequential(nn.Conv2d(channel, channel, 1, 1, bias=False), nn.BatchNorm2d(channel),
                                          nn.ReLU(inplace=True))
        ...
        if flag == 0:
            ...
            self.tf_zx = nn.Sequential(
                nn.Conv1d(channel * 3, channel, kernel_size=1, stride=1, groups=channel, bias=False),
                nn.BatchNorm1d(channel), nn.LeakyReLU(0.2, inplace=True))
        else:
            self.tf_zx = nn.Sequential(
                nn.Conv1d(channel * 3, channel, kernel_size=1, stride=1, groups=channel, bias=False),
    ...
    def forward(self, position, identity, y):  # y b,d,g,1
        ...
        zx = self.z_x(y)
        del y
        zx = torch.cat([zx * torch.sin(theta_x_beta) * torch.cos(theta_x_alpha),
                        zx * torch.sin(theta_x_beta) * torch.sin(theta_x_alpha), zx * torch.cos(theta_x_beta)],
                       dim=-2).reshape(b, -1, g, k)  # b,out*3,g
