...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        self.layers_lora = nn.ModuleList()
        for n, k in zip([input_dim] + h, h + [output_dim]):
            self.layers_lora.append(nn.Linear(n, k // 4))
            self.layers_lora.append(nn.Linear(k // 4 , k))

    def forward(self, x):
        for i, layer in enumerate(self.layers):
            x = F.gelu(layer(x) + self.layers_lora[i * 2 + 1](
                self.layers_lora[i * 2](x))) if i < self.num_layers - 1 else layer(x) + self.layers_lora[i * 2 + 1](
                self.layers_lora[i * 2](x))
...
class EncoderDecoderLora(nn.Module):
    ...
    def forward(self, src, mask, support_embed, pos_embed, support_order_embed,
        ...
        support_embed_ = support_embed.transpose(0, 1)
        mask = mask.flatten(1)

        query_embed = src
        refined_support_embed = support_embed_
        ...
        return outs_dec.transpose(1, 2), initial_proposals_for_loss, out_points, similarity_map, query_embed
