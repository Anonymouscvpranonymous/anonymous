...
class CustomMultiLoss(nn.Module):
    def __init__(self, nb_outputs=3):
        ...
        self.log_vars = nn.ParameterList([nn.Parameter(torch.zeros(1).requires_grad_()) for _ in range(self.nb_outputs)])
    ...
    def forward(self, y, y_pred, y_shape, y_shape_pred, y_pose, y_pose_pred):
        ...
        for y_true, y_pred, log_var in zip(ys_true, ys_pred, self.log_vars):
            ...
            loss += torch.sum(precision * (y_true - y_pred) ** 2. + log_var[0], -1)
...
#             precision = torch.exp(-log_var[0])
#             vars2 = (precision ** 2)
#             loss += torch.sum(0.5 * vars2 * (y_true - y_pred) ** 2. + torch.log(vars2 + 1), -1)
#         return torch.mean(loss)