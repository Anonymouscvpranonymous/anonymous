...
class DistributedDataParallelWrapper(nn.Module):
    """A DistributedDataParallel wrapper for models_ in 3D mesh estimation task.

    In  3D mesh estimation task, there is a need to wrap different modules in
    the models_ with separate DistributedDataParallel. Otherwise, it will cause
    ...
    1. Wrap the modules in the models_ with separate MMDistributedDataParallel.
    ...
    def to_ddp(self, device_ids, dim, broadcast_buffers,
        ...
        """Wrap models_ with separate MMDistributedDataParallel.
    ...
    def scatter(self, inputs, kwargs, device_ids):
        ...
        Args:
            ...
            kwargs (dict): Args for
                ``pam_mmcv.parallel.scatter_gather.scatter_kwargs``.
