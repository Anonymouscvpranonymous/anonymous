...
from common.nets.module import PositionNet, HandRotationNet, FaceRegressor, BoxNet, BoxSizeNet, HandRoI, FaceRoI, \
    BodyRotationNet
...
class Model(nn.Module):
    ...
    def get_coord(self, root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, shape, expr, cam_trans, mode):
        ...
        output_zero_pose = self.smplx_layer(betas=shape, body_pose=torch.zeros_like(body_pose), global_orient=torch.zeros_like(root_pose),
                                            right_hand_pose=torch.zeros_like(rhand_pose),
                                            left_hand_pose=torch.zeros_like(lhand_pose), jaw_pose=torch.zeros_like(jaw_pose), leye_pose=zero_pose,
                                            reye_pose=zero_pose, expression=expr)
        mesh_cam_zero_pose = output_zero_pose.vertices + cam_trans[:, None, :]
        ...
        return joint_proj, joint_cam, mesh_cam, mesh_cam_zero_pose, joint_origin_cam , face_cam
    ...
    def generate_mesh_gt(self, targets, mode):
        ...
        # final output
        joint_proj, joint_cam, mesh_cam, mesh_cam_zero_pose, joint_origin_root_cam, face_cam  = self.get_coord(root_pose, body_pose, lhand_pose,
                                                                                       rhand_pose, jaw_pose, shape,
                                                                                       expr, cam_trans, mode)
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        root_pose, body_pose, shape, cam_param, = self.body_regressor(body_pose_token, shape_token, cam_token,
        ...
        rhand_bbox = restore_bbox(rhand_bbox_center, rhand_bbox_size, cfg.input_hand_shape[1] / cfg.input_hand_shape[0],
                                  2.0).detach()  # xyxy in (cfg.input_body_shape[1], cfg.input_body_shape[0]) space
        face_bbox = restore_bbox(face_bbox_center, face_bbox_size, cfg.input_face_shape[1] / cfg.input_face_shape[0],
                                 1.5).detach()  # xyxy in (cfg.input_body_shape[1], cfg.input_body_shape[0]) space
        ...
        hand_feats = self.hand_roi_net(img_feat, lhand_bbox,
                                       rhand_bbox)  # list, hand_feat: flipped left hand + right hand
        ...
        _, hand_joint_img, hand_img_feat_joints = self.hand_position_net(
            hand_feats[-2])  # (2N, J_P, 3) in (hand_hm_shape[2], hand_hm_shape[1], hand_hm_shape[0]) space
        ...
        hand_img_feat_joints = self.hand_decoder(hand_feats, coord_init=hand_coord_init.detach(),
                                                 query_init=hand_img_feat_joints)
        # hand regression head
        hand_pose = self.hand_regressor(hand_img_feat_joints, hand_joint_img.detach())
        hand_pose = rot6d_to_axis_angle(hand_pose.reshape(-1, 6)).reshape(hand_img_feat_joints.shape[0],
                                                                          -1)  # (2N, J_R*3)
        ...
        _, face_joint_img, face_img_feat_joints = self.face_position_net(
            face_feats[-2])  # (N, J_P, 3) in (face_hm_shape[2], face_hm_shape[1], face_hm_shape[0]) space
        face_coord_init = self.heatmap2norm(face_joint_img, cfg.output_face_hm_shape)
        face_img_feat_joints = self.face_decoder(face_feats, coord_init=face_coord_init.detach(),
                                                 query_init=face_img_feat_joints)
        ...
        # final output
        joint_proj, joint_cam, mesh_cam, mesh_cam_zero_pose, joint_origin_root_cam, face_cam = self.get_coord(root_pose, body_pose,
                                                                                          lhand_pose,
                                                                                          rhand_pose, jaw_pose, shape,
                                                                                          expr, cam_trans, mode)
        ...
        if mode == 'train':
            ...
            out['smplx_mesh_cam_zero_pose'] = mesh_cam_zero_pose
            ...
            out['face_cam'] = face_cam
            # root-relative 3D coordinates
            out['joint_origin_root_cam'] = joint_origin_root_cam
...
def get_model(mode):
    ...
    decoder_cfg = Config.fromfile(
        os.path.join(cfg.root_dir, 'main/transformer_utils/configs/osx/decoder/hand_decoder.py'))
    ...
    decoder_cfg = Config.fromfile(
        os.path.join(cfg.root_dir, 'main/transformer_utils/configs/osx/decoder/face_decoder.py'))
